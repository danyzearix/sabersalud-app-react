...
import { CURSOS_ESTETICA } from ".//../../data/cursosestetica.js";
...
const CertificadosEstetica = () => {
  ...
  const [cursosDisponibles] = useState(() => [...CURSOS_ESTETICA]);
  const [searchStatus, setSearchStatus] = useState("idle"); // idle | searching | found | not_found

  // Tema: dark/light (idéntico al componente anterior)
  ...
  // Habilitar/Deshabilitar interacción según resultado de búsqueda
  const userFound =
    searchStatus === "found" &&
    userData &&
          ...
          {userData && userData.map((user) => (
            <View key={user.id} style={{ flexDirection: 'row', flex: 1 }}>
              ...
              <View style={styles.column}>
                ...
                {/* Puedes reactivar el texto de validez si tus certificados de estética también lo requieren */}
