...
import { CURSOS_SALUD } from ".//../../data/cursossalud.js";

// Imagen de fondo PDF
const backgroundImage = 'https://sabersalud.co/wp-content/uploads/2025/10/CERTIFICADO-SABERSALUD-2025-scaled.jpg';

// React-PDF: fuentes y estilos internos (no afectan el DOM)
...
const styles = StyleSheet.create({
  ...
  textodos: { textAlign: 'left', fontFamily: "Montserrat", fontSize: 10, marginTop: 100, marginHorizontal: 10, color:'#ffffff' },
  ...
  dropdown: { textAlign: 'center', fontSize: 16, marginTop: 10 },
  backgroundImage: { position: 'absolute', width: '100%', height: '100%' },
  timestamp: { textAlign: 'center', fontSize: 8, color:'#ffffff', marginTop: 6 },
  textoconcordancia: { textAlign: 'left', fontSize: 9, fontFamily: "Montserrat", marginHorizontal: 10, whiteSpace: 'pre-wrap', lineHeight: 1, color:'#ffffff' },
});

// ====== Componente principal con modo oscuro + validación de usuario ======
const Certificados = () => {
  ...
  const [cursosDisponibles] = useState(() => [...CURSOS_SALUD]);
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        // Considera "no encontrado" si array vacío o respuesta inválida
        const found =
        ...
        setUserData([]);           // explícitamente sin resultados
    ...
    if (numeroId.length >= 6) {
      ...
      // Reinicia estado si el ID es corto
      setUserData(null);
  ...
  // ¿Hay usuario encontrado?
  const userFound =
    searchStatus === "found" &&
    userData &&
    ((Array.isArray(userData) && userData.length > 0) || (!Array.isArray(userData) && userData));

  // Nota: pediste habilitar basado SOLO en "resultado de búsqueda".
  // Si quisieras exigir campos completos antes de habilitar, podríamos AND con: selectedOption?.nombre && selectedDate && courseValue && invoiceDate
    ...
    <div className="page-wrap">
      ...
      <div className='container'>
        ...
        {searchStatus === "searching" && (
          <p className="status status-info">Buscando usuario…</p>
        )}
        {searchStatus === "not_found" && (
          <p className="status status-error">No se encontró un usuario con ese ID.</p>
        )}
        {searchStatus === "found" && (
          <p className="status status-ok">Usuario encontrado ✅</p>
        )}
        ...
        {/* Botón PDF: si no hay usuario, renderizamos un botón deshabilitado para feedback visual */}
        {canInteract ? (
          <PDFDownloadLink
